...
from llmbroker.data.common import get_stktype_list, get_new_holidays
from llmbroker.util import llmbroker_debug


def is_exist_db(connect):
   
    cur = connect.cursor()
    cur.execute("SELECT 1 FROM information_schema.SCHEMATA where SCHEMA_NAME='llmbroker_base'")
    a = cur.fetchone()
    cur.close()
...
def get_db_version(connect):
    try:
        cur = connect.cursor()
        cur.execute("select `version` from `llmbroker_base`.`version`")
        a = cur.fetchone()
        cur.close()
...
def get_marketid(connect, market):
    cur = connect.cursor()
    cur.execute("select marketid, market from `llmbroker_base`.`market` where market='{}'".format(market.upper()))
...
def get_codepre_list(connect, marketid, quotations):
    ...
    sql = "select codepre, type from `llmbroker_base`.`coderuletype` " \
...
def get_stock_list(connect, market, quotations):
    ...
    stktype_list = get_stktype_list(quotations)
    sql = "select stockid, marketid, code, valid, type from `llmbroker_base`.`stock` where marketid={} and type in {}"\
...
def import_new_holidays(connect):
    """导入新的交易所休假日历"""
    cur = connect.cursor()
    cur.execute("select date from `llmbroker_base`.`holiday` order by date desc limit 1")
    a = cur.fetchall()
    last_date = a[0][0] if a else 19901219
    holidays = get_new_holidays()
    new_holidays = [(int(v), ) for v in holidays if int(v) > last_date]
    if new_holidays:
        cur.executemany("insert into `llmbroker_base`.`holiday` (date) values (%s)", new_holidays)
        connect.commit()
        cur.close()
...
def update_extern_data(connect, market, code, data_type):
    ...
    for index_type in index_list:
        llmbroker_debug("{}{} update {} index".format(market, code, index_type))
